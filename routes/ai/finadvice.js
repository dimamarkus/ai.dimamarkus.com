...
app.post('/business/finadvice', async (req, res, next) => {
	let { content, currentPrompt, title, salary, skills, company, contact } = req.body

	let prompt = ""
	let inputRaw = ""

	if(currentPrompt === "Basic Advice"){
		prompt = `Get detailed advice from the text:\n###\n` +
	...
	if(currentPrompt === "Detailed Advice"){
		prompt = `Get detailed advice from the following details:\n###` +
	...
	const gptResponse = await openai.complete({
		...
		user: req.user._id,
		stream: false,
		stop: ["###", "<|endoftext|>","FINANCIAL ADVICE","TEXT" ],
	});
